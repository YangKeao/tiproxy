...
- code paths that still need a unique Prometheus source or etcd client during the transition to multi-cluster-aware callers
...
- fall back to process-default HTTP and TCP behavior when the backend has no cluster label or the cluster is missing
...
- inject synthetic label `config.ClusterLabelName = clusterName`
- merge the resulting topology map by backend address

If the same backend address appears from two clusters, the first one wins and TiProxy logs a warning.

This synthetic cluster label is the key that allows downstream routing and network dispatch to remain cluster-aware without redesigning all observer and router interfaces.

## Router and Routing Keys

### Router-Side Cluster Awareness

`router.BackendInst` now exposes cluster identity:
...
`backendWrapper.ClusterName()` reads the cluster name from backend labels. Those labels come from the aggregated topology path described above.
...
- backend labels also carry `cluster-name`
- `ScoreBasedRouter` sets `matchType = MatchPort` when `balance.routing-rule = "port"`
- group membership is computed from existing backend labels
...
3. if `PrimaryCluster()` is non-nil, expose its etcd client as the server's single-cluster etcd client
4. use `MetricsQuerier()` as the server-wide `metricsreader.MetricsReader`
...
7. enable VIP only when `PrimaryCluster()` returns a cluster
...
`DefaultHealthCheck` uses the backend's synthetic cluster label to choose:
