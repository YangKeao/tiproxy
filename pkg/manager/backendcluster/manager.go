...
func (c *Cluster) GetTiDBTopology(ctx context.Context) (map[string]*infosync.TiDBTopologyInfo, error) {
	if c == nil || c.infoSyncer == nil {
		return map[string]*infosync.TiDBTopologyInfo{}, nil
	}
...
func normalizeCluster(cluster config.BackendCluster) config.BackendCluster {
	...
	cluster.NSServers = strings.TrimSpace(cluster.NSServers)
	return cluster
}
...
func clusterReusable(cluster *Cluster, cfg config.BackendCluster) bool {
	...
	return normalizeCluster(cluster.cfg) == normalizeCluster(cfg)
}
	...
	for clusterName, cluster := range clusters {
		...
		for addr, info := range infos {
			if oldInfo, ok := merged[addr]; ok {
				m.lg.Warn("duplicate backend from different clusters, keep the first one",
					zap.String("addr", addr),
					zap.String("cluster", clusterName),
					zap.String("first_cluster", oldInfo.Labels[config.ClusterLabelName]))
				continue
			}
			cloned := *info
